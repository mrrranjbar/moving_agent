...
class PPO_discrete:
    ...
    def update(self, replay_buffer, total_steps):
        ...
        """
        # mrr
        actor_loss_data_saver = []
        critic_loss_data_saver = []
        # mrr
        ...
        for _ in range(self.K_epochs):
            ...
            for index in BatchSampler(SubsetRandomSampler(range(self.batch_size)), self.mini_batch_size, False):
                ...
                #mrr
                actor_loss_data_saver.append(actor_loss.mean().item())
                ...
                #mrr
                critic_loss_data_saver.append(critic_loss.item())
        ...
        #mrr
        return actor_loss_data_saver, critic_loss_data_saver
    ...
    def load_models(self, actor_path, critic_path, is_eval=True):
        self.actor.load_state_dict(torch.load(actor_path))
        self.critic.load_state_dict(torch.load(critic_path))
        # Count and print the number of parameters in the actor and critic
        num_actor_params = sum(p.numel() for p in self.actor.parameters())
        num_critic_params = sum(p.numel() for p in self.critic.parameters())

        print(f"Number of actor model parameters: {num_actor_params}")
        print(f"Number of critic model parameters: {num_critic_params}")
        if is_eval:
            self.actor.eval()
            self.critic.eval()
        else:
