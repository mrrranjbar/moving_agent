...
from PPO_Continues.classic_approach.PathFromTarget import PathFromTarget
...
class Process:
    ...
    def initialize(self, Rings: List[LinkList], src, length, alpha, width, height):
        self.AllVertexes.clear()
        ...
        # pr.chain.points.append(src)
        pr.chain.id = 0
        pr.chain.minAngle = 360.0 - self.Alpha
        pr.chain.maxAngle = 360.0 + self.Alpha
        pr.vt = v
        v.pre.append(pr)
        self.source = v
